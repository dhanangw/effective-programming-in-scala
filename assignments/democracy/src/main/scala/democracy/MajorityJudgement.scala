...
object Grade:
  ...
  def median(grades: Seq[Grade]): Grade =
    val sequenceSize: Int = grades.size
    val medianGrade: Grade = grades.sortBy(_.ordinal).apply(sequenceSize / 2)
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestMedianGrade: Grade =
        gradesPerCandidate.values.filter(_.nonEmpty).map(Grade.median).maxBy(_.ordinal)
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate.filter((key, values) => values.contains(bestMedianGrade))
