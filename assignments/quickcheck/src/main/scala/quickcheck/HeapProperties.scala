...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val meldedHeap: List[Node] = {
		val maxValue: Int = x.max(y)
		val minValue: Int = x.min(y)
		val heap1: List[Node] = insert(maxValue, insert(maxValue, empty))
		val heap2: List[Node] = insert(minValue, insert(minValue, empty))
		meld(heap1, heap2)
		}
      ...
      val deleteTwoMinAndFindMin: Boolean = {
        val deletedMinTwiceHeap = deleteMin(deleteMin(meldedHeap))
        if !isEmpty(deletedMinTwiceHeap) then
          findMin(deletedMinTwiceHeap) == x.max(y)
        else
          false
      }
      ...
      val insertMinAndFindMin: Boolean = {
        val minValue = x.min(y)
        findMin(insert(minValue, meldedHeap)) == minValue
      }
  ...
  val meldingHeaps: (String, Prop) =
    def checkMeldingHeap(heap1: List[Node], heap2: List[Node], meldedHeap: List[Node]): Boolean = {
      if (isEmpty(heap1) && isEmpty(heap2) && isEmpty(meldedHeap)) then
        true
      else
        val minHeap1 = {
        ...
        val minMeldedHeap: Int = findMin(meldedHeap)

        if minHeap1 != null && minHeap1 == minMeldedHeap then
          checkMeldingHeap(deleteMin(heap1), heap2, deleteMin(meldedHeap))  
        else if minHeap2 != null && minHeap1 != minMeldedHeap then
          checkMeldingHeap(heap1, deleteMin(heap2), deleteMin(meldedHeap))  
        else
          false
    }
    ...
    forAll { (heap1: List[Node], heap2: List[Node]) =>
      checkMeldingHeap(heap1, heap2, meld(heap1, heap2))
    }
