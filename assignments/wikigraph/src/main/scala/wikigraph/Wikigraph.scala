...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap {
      linkedIds => WikiResult.traverse(linkedIds.toSeq)(client.nameOfArticle).map(_.toSet)
    }
        ...
        case Some(((d, elem), newQueue)) => 
          if d >= maxDepth then
            ...
            client
              .linksFrom(elem)
              .flatMap{ linkedIds =>
                if linkedIds.contains(target) then
                  WikiResult.successful(Some(d))
                else
                  iter(
                    visited + elem,
                    newQueue.enqueueAll(
                      linkedIds.filter(!visited.contains(_)).map((d + 1, _))
                    )
                  )
              }.fallbackTo(iter(visited + elem, newQueue))
      }
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    WikiResult.traverse(pairs) {
      (srcTitle, dstTitle) =>
        client.searchId(srcTitle).zip(client.searchId(dstTitle)).flatMap {
          (srcId, dstId) =>
            breadthFirstSearch(srcId, dstId, maxDepth).flatMap {
              optDist => WikiResult.successful(srcTitle, dstTitle, optDist)
            }
        }
    }
end Wikigraph
